...
kpiTitleCompact: {
  ...
  paddingRight: 34,   // reserve space under the stars on the right
      ...
      tickCellTxt: { fontSize: 10, fontWeight: '800' },
...
>
  ...
  style={S.iconBtn}
  onPress={() => {
    const firstXI = (current?.picks || []).slice(0, 11)[10];
    if (firstXI != null) {
      // statsMode=true (so the market knows we came from the Stats button)
      // anyPos=true   (don’t restrict by outgoing player's position)
      setTransferMode({ outId: firstXI, outIdx: 0, statsMode: true, anyPos: true });
      setMarketOpen(true);
    }
  }}
