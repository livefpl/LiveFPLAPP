// prices_v2.js — Add Predictions/Actual top-level tab (Actual = historical price changes)
// Keeps your existing Predictions UI (All Players / Watchlist / My Team) exactly as-is,
// and adds a new "Actual" tab that pulls https://livefpl.us/api/price_changes_api.json
//
// Actual tab features:
// - Prominent "Latest" day block first
// - Full history grouped by date (newest -> oldest)
// - Search by player/team
// - Filter: All / Risers / Fallers
// - Sorting within each day: UP first, then DOWN; highest price first
...
const buildEOMapNear = (json) => {
  ...
  if (Array.isArray(json)) {
    for (const row of json) if (row && typeof row === 'object') setOne(row?.element ?? row?.id ?? row?.element_id ?? row?.player_id, row);
  }
  return map;
};
...
const StatusPill = ({ bucket, styles, isDark }) => {
  ...
  return (
    <View style={[styles.pillBase, pillStyle]}>
      {showArrow && (
        <Image source={bucket.includes('up') ? assetImages.up : assetImages.down} style={{ width: 10, height: 10, resizeMode: 'contain' }} />
      )}
...
const tableRowS = (C, isDark) =>
  StyleSheet.create({
    ...
    cellInfo: { flex: 2.4, minWidth: 0, flexDirection: 'row', alignItems: 'center', gap: 8 },
  ...
  <TouchableOpacity onPress={onPress} activeOpacity={0.85} hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}>
    <View
      style={[
        ...
        active
          ? isDark
            ? { backgroundColor: '#1b2a4a', borderColor: '#3b82f6' }
      ...
      >
        <ScrollView keyboardShouldPersistTaps="handled">
          {suggestions.slice(0, 12).map((p, i) => (
            <TouchableOpacity key={`${p._el}|sugg`} activeOpacity={0.85} onPress={() => onPick?.(p)}>
              <View
                style={{
                  flexDirection: 'row',
                  alignItems: 'center',
                  ...
                  borderColor: isDark ? C.border : '#f1f5f9',
                }}
              >
...
const PriceDetailsModal = ({ visible, onClose, player, starred, onToggleStar, C, isDark }) => {
  ...
  const chipStyle = (activeBg, activeBr) => [sheetS.chip, { backgroundColor: activeBg, borderColor: activeBr }];
        ...
        <View style={[sheetS.card, { backgroundColor: bg, borderColor: br }]}>
          <View style={[sheetS.headerRow, { backgroundColor: strip, borderBottomWidth: 1, borderColor: br }]}>
            ...
            <View style={sheetS.titleBox}>
              <Text style={[sheetS.name, { color: ink }]} numberOfLines={1}>
                {player.name}
              </Text>
...
const Tabs = ({ value, onChange, C, isDark }) => {
  const Tab = ({ id, label }) => {
    ...
    return (
      <TouchableOpacity onPress={() => onChange(id)} activeOpacity={0.85} style={{ flex: 1 }} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
        <View
          style={{
            paddingVertical: 10,
            borderBottomWidth: 3,
            borderColor: active ? (C.accent || '#6366f1') : 'transparent',
    ...
    return (
      <TouchableOpacity
        ...
        style={{
          ...
          alignItems: 'center',
          borderWidth: 1,
          backgroundColor: active ? (isDark ? C.accentPillBg : C.accent) : (isDark ? '#0b1224' : '#ffffff'),
          borderColor: active ? C.accent : (isDark ? '#1e2638' : '#e2e8f0'),
        }}
      >
        <Text style={{ fontWeight: '900', color: active ? (C.accentOn || '#fff') : (isDark ? C.ink : '#0f172a') }}>{label}</Text>
      </TouchableOpacity>
    );
  };
  return (
  ...
  return (
    <View style={{ flexDirection: 'row', gap: 10 }}>
      ...
      <Card title={`Predicted Falls tonight (${down.length})`}>
        ...
        <Toggle open={openHidden.main} onPress={() => setOpenHidden((s) => ({ ...s, main: !s.main }))} count={downHiddenMain.length} />
        ...
        <Toggle open={openHidden.maybe} onPress={() => setOpenHidden((s) => ({ ...s, maybe: !s.maybe }))} count={maybeHiddenDown.length} />
      </Card>
    </View>
  );
};
...
const ViewModeToggle = ({ mode, onChange, disabled, C, isDark, compact = false }) => {
  const Base = ({ id, label }) => {
    ...
    return (
      ...
      >
        <Text style={{ fontWeight: '800', fontSize: 12, color: active ? (C.accentOn || '#fff') : isDark ? C.ink : '#0f172a' }}>{label}</Text>
      </TouchableOpacity>
    );
  };
...
/* ───────── Overview / Watchlist / MyTeam (your code) ───────── */
/* (kept as-is from your message; only edits inside PricesV2 at bottom to add top-level mode switch) */
      ...
      {(() => {
        ...
        return (
          <>
            ...
            {showEOChip && (
              <Btn active={!!data.onlyAbove1Pct} onPress={() => data.setOnlyAbove1Pct(!data.onlyAbove1Pct)}>
                <Text
                  style={{
                    fontWeight: '800',
                    fontSize: 12,
...
/* ───────── New: Actual changes tab ───────── */
const ACTUAL_URL = 'https://livefpl.us/api/price_changes_api.json';
const ACTUAL_CACHE_KEY = 'prices.actual.cache.v1';
...
const sortActualRows = (rows) => {
  return [...rows].sort((a, b) => {
    const da = a.direction === 'up' ? 0 : 1;
    const db = b.direction === 'up' ? 0 : 1;
    if (da !== db) return da - db;

    const pa = Math.max(Number(a.old_price ?? 0), Number(a.new_price ?? 0));
    const pb = Math.max(Number(b.old_price ?? 0), Number(b.new_price ?? 0));
...
const groupByDate = (rows) => {
  ...
  return dates.map((d) => ({ date: d, rows: sortActualRows(map.get(d)) }));
};

const ActualRow = ({ item, C, isDark }) => {
  const isUp = item.direction === 'up';
  const oldP = Number(item.old_price ?? 0);
  const newP = Number(item.new_price ?? 0);
  const delta = (newP - oldP).toFixed(1);
  const deltaColor = isUp ? (isDark ? '#86efac' : '#166534') : (isDark ? '#fecaca' : '#991b1b');
  ...
  return (
    ...
    >
      <ClubCrest id={Number(item.team_id)} style={{ width: 22, height: 22 }} resizeMode="contain" />
      <View style={{ flex: 1, minWidth: 0 }}>
        <Text style={{ color: isDark ? C.ink : '#0f172a', fontWeight: '900' }} numberOfLines={1}>
          {item.player_name}
        </Text>
      ...
      <View style={{ alignItems: 'flex-end' }}>
        <Text style={{ color: isDark ? C.whiteMd : '#334155', fontWeight: '800', fontSize: 12 }} numberOfLines={1}>
          {money(oldP)} → {money(newP)}
        </Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
  <Image
    source={isUp ? assetImages.up : assetImages.down}
    style={{ width: 10, height: 10, resizeMode: 'contain', marginRight: 4 }}
  />
  <Text style={{ color: deltaColor, fontWeight: '900' }}>
    {`${isUp ? '+' : ''}${delta}`}
  </Text>
</View>
...
function ActualTab({ ui }) {
  ...
  const load = useCallback(async (force = false) => {
    try {
      ...
      if (!force) {
        const raw = await AsyncStorage.getItem(ACTUAL_CACHE_KEY);
        if (raw) {
          const cached = JSON.parse(raw);
          if (cached?.t && cached?.data && Date.now() - cached.t < ACTUAL_TTL_MS) {
      ...
      const res = await fetch(ACTUAL_URL, { headers: { 'cache-control': 'no-cache' } });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      setPayload(json);
      AsyncStorage.setItem(ACTUAL_CACHE_KEY, JSON.stringify({ t: Date.now(), data: json })).catch(() => {});
    } catch (e) {
  ...
  const grouped = useMemo(() => {
    ...
    if (dir !== 'all') arr = arr.filter((r) => r.direction === dir);
  ...
  const SegBtn = ({ id, label }) => {
    ...
    return (
      ...
      >
        <Text style={{ fontWeight: '900', fontSize: 12, color: active ? (C.accentOn || '#fff') : (isDark ? C.ink : '#0f172a') }}>{label}</Text>
      </TouchableOpacity>
    );
  };
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        ...
        >
          <Text style={{ color: isDark ? C.ink : '#0f172a', fontWeight: '900' }}>{refreshing ? '…' : '↻'}</Text>
        </TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: BOTTOM_INSET }} keyboardShouldPersistTaps="handled">
        {/* Latest block */}
        {latest ? (
          <View style={{ marginBottom: 14 }}>
            ...
            {latest.rows.slice(0, 10).map((r, idx) => (
              <ActualRow key={`${r.date}-${r.player_id}-${r.direction}-${idx}`} item={r} C={C} isDark={isDark} />
            ))}
...
export default function PricesV2() {
  ...
  useFocusEffect(
  useCallback(() => {
    // Always land on Predictions when opening the screen
    setMode('pred');
    setTab('overview');
    setSearchQuery('');
    setViewMode('summary');
  ...
  // NEW: top-level mode
  const [mode, setMode] = useState('pred'); // 'pred' | 'actual'
  ...
  const onSearchChange = (text) => {
    ...
    if (text.trim().length > 0 && prev.trim().length === 0) {
      ...
      // viewMode flip handled in OverviewTab
    }
  };
  ...
  // When switching Pred/Actual: clear prediction-only search state so Actual feels clean.
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: C.bg }} edges={['left', 'right']}>
      ...
      <View style={{ flex: 1 }}>
        <View style={{ flex: 1, paddingHorizontal: 10 }}>
          ...
          <TopModeToggle mode={mode} onChange={handleModeChange} C={C} isDark={isDark} />
